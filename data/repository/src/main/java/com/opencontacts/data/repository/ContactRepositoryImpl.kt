...
class ContactRepositoryImpl @Inject constructor(
    ...
    override suspend fun upsertTag(vaultId: String, tag: TagSummary) {
        val dao = vaultDatabaseFactory.getDatabase(vaultId).contactsDao()
        dao.upsertTag(TagEntity(tagName = tag.name, displayName = tag.name, colorToken = tag.colorToken, createdAt = System.currentTimeMillis()))
    }

    override suspend fun upsertFolder(vaultId: String, folder: FolderSummary) {
        val dao = vaultDatabaseFactory.getDatabase(vaultId).contactsDao()
        dao.upsertFolder(FolderEntity(folderName = folder.name, displayName = folder.name, iconToken = folder.iconToken, colorToken = folder.colorToken, createdAt = System.currentTimeMillis()))
    }
